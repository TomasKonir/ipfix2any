        ...
        if (!v.isObject()) {
            ...
            if (!failed) {
                ...
                if (type == "signed") {
                    ...
                    fdt.convertFunc = convertUnsignedNumber;
                } else if (type == "icmp6_type") {
                    fdt.convertFunc = convertUnsignedNumber;
                } else if (type == "icmp6_code") {
                    fdt.convertFunc = convertUnsignedNumber;
        ...
        for (const auto& v : outputs) {
            ...
            for (const QJsonValue& v : filterArray) {
                ...
                if (name == "fields") {
                    ...
                    filterList << new FilterSkip(o.value("params").toObject());
                } else if (name == "removeZero") {
                    filterList << new FilterZero(o.value("params").toObject());
                } else {
...
unsigned long IPFIX::hdrGet(ipfix_hdr_t& hdr, const char* data) {
    ...
    if (debug)
        qInfo() << "IPFIX:" << hdr.exportTime << hdr.length << hdr.observationDomainID << hdr.sequenceNumber << hdr.version;
...
void IPFIX::processTemplates(const char* data, long remain, QString ident) {
    ...
    while (remain > hdrSize) {
        ...
        if (debug)
            qInfo() << "TEMPLATE:" << hdr.templateID << hdr.fieldCount;
        data += sizeof(hdr);
        remain -= sizeof(hdr);
        t.baseSize = 0;
        for (int i = 0; i < hdr.fieldCount; i++) {
            ...
            if (debug)
                qInfo() << "ELEMENT:" << enterprise << element.informationElementId << element.fieldLength;
